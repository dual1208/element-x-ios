...
The base fork was validated on September 18, 2026 with Xcode 27.0 and iOS 26.7: the Debug app built successfully, signed as `GQZ5664B67.com.dual1208.elementx`, installed and launched on the gated physical iPhone, completed the static MAS custom-scheme authorization flow, and reached the signed-in Chats screen. For the managed-family revision, the Debug app target compiled on September 20, 2026 during the gated focused-test build. The corrected focused tests still need a complete physical-device run while the shared heavy-build slot is available. A direct install of the 582 MB Debug app stalled in CoreDevice, but the 301 MB Release app re-exported with development signing installed over the existing app without removing its container. The device then reported build 2, and the gated launch plus process check succeeded. Use `just export-device-archive` followed by `just install-device-archive` to reproduce this smaller physical install path.
...
For App Store preparation, `just archive` creates a Release archive for a generic iOS device, `just export-archive` exports it with `Config/AppStoreExportOptions.plist`, and `just validate-archive` asks Apple's service to validate that archive without submitting it for review. Override `IOS_ARCHIVE_PATH`, `IOS_EXPORT_PATH`, `IOS_EXPORT_OPTIONS_PLIST`, or `IOS_VALIDATION_OPTIONS_PLIST` when needed. On September 20, 2026, the archive and export recipes succeeded through automatic provisioning and produced an App Store Connect IPA signed by the team's Apple cloud-managed Distribution certificate. The exported app has `aps-environment=production`, `get-task-allow=false`, the fork's app group and keychain group, and only `applinks:matrix.to` as its associated domain. Validation reached App Store Connect but stopped because no app record exists for `com.dual1208.elementx`; create that record before retrying. Export prepares the artifact but does not upload or submit it.
