...
struct HomeScreenEmptyStateView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            Text(context.viewState.isManagedFamilyMode ? ManagedFamilyL10n.managedFamilySetupNeedsHelp : L10n.screenRoomlistEmptyTitle)
                .font(.compound.bodyLG)
                .foregroundColor(.compound.textSecondary)
                .multilineTextAlignment(.center)
            
            Text(context.viewState.isManagedFamilyMode ? ManagedFamilyL10n.managedFamilySetupNeedsHelpMessage : L10n.screenRoomlistEmptyMessage)
                .font(.compound.bodyLG)
                .foregroundColor(.compound.textSecondary)
                .multilineTextAlignment(.center)
                .padding(.bottom, 12)
            ...
            if !context.viewState.isManagedFamilyMode {
                ...
                Button { context.send(viewAction: .showSettings) } label: {
                    Label(L10n.actionOpenSettings, icon: \.settings)
                        .font(.compound.bodyLGSemibold)
                        .foregroundColor(.compound.textOnSolidPrimary)
                        .padding(.vertical, 6)
                        .padding(.horizontal, 22)
